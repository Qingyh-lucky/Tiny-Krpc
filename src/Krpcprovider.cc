...
void KrpcProvider::NotifyService(google::protobuf::Service *service) {
    ...
    // 打印服务名
    KrpcLogger::Info("service_name=" + service_name);
...
void KrpcProvider::Run() {
    // 读取配置文件中的RPC服务器IP和端口(这个是自己，当前rpcprovider，一个服务器可能有多个网卡)
    ...
    for (auto &sp : service_map) {
        ...
        for (auto &mp : sp.second.method_map) {
            ...
            // 先确保方法目录是持久节点
            zkclient.Create(method_path.c_str(), nullptr, 0);  // 持久节点

            // 真正注册自己地址的是子节点
            char method_path_data[128] = {0};
            sprintf(method_path_data, "%s:%d", ip.c_str(), port);  // 当前服务节点地址

            std::string method_addr_path = method_path + "/" + method_path_data;  // /UserServiceRpc/Login/192.168.1.10:8000

            zkclient.Create(method_addr_path.c_str(), method_path_data, strlen(method_path_data), ZOO_EPHEMERAL);
        }
    }
...
//已建立连接用户的读写事件回调，如果远程有一个rpc服务的调用请求，则OnMessage方法就会响应
