...
void ZkClient::Start() {
    ...
    */
    ...
    std::cout<< "zookeeper_init success";  // 记录日志，表示连接成功
}
...
void ZkClient::Create(const char *path, const char *data, int datalen, int state) {
    ...
    if (flag == ZNONODE) {  // 如果节点不存在
        ...
        if (flag == ZOK) {  // 创建成功
            ...
            std::cout  << "znode create failed... path:" << path;
            exit(EXIT_FAILURE);  // 退出程序
        }
    }
}
...
std::vector<std::string> ZkClient::GetChildren(const std::string &path) {
    ...
    int flag = zoo_get_children(m_zhandle, path.c_str(), 0, &strings);
    if (ZOK == flag) {
        for (int i = 0; i < strings.count; ++i) {
            children.push_back(strings.data[i]);  // 比如 "192.168.1.10:8000"
        }
        deallocate_String_vector(&strings);
    } else {
         std::cout << "get children failed! path: " << path;
    }
    return children;
}